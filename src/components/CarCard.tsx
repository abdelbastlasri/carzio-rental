  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-lg transition overflow-hidden flex flex-col">
      <div className="bg-gray-100 h-48 flex items-center justify-center">
        {car.image ? (
          <img src={car.image} alt={car.name} className="w-full h-full object-contain p-4" />
        ) : (
      ...
      <div className="p-5 flex-1 flex flex-col">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-heading font-bold text-lg text-black">{car.name}</h3>
          <span className="text-gold font-heading font-bold text-xl">{car.pricePerDay}€<span className="text-gray-400 text-xs font-normal">/day</span></span>
        </div>
        <p className="text-gray-400 text-xs mb-3">or similar &middot; {car.type}</p>
        ...
        <div className="border-t border-gray-100 pt-3 mt-auto">
          <p className="text-xs text-gray-400 font-medium mb-2">Extras:</p>
          ...
          <button
            ...
            className="w-full bg-burgundy hover:bg-red-900 text-white font-semibold text-sm py-2.5 rounded-lg transition"
          >
            Book Now
          </button>
        </div>
