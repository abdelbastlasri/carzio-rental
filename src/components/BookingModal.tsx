...
export default function BookingModal({ isOpen, onClose, preselectedCar, preselectedLocation }: BookingModalProps) {
  const localDate = () => {
    const d = new Date();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 md:pt-16 overflow-y-auto">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-2xl mx-4 shadow-2xl z-10 mb-10">
        ...
        <div className="flex items-center justify-between p-5 border-b border-gray-100">
          <div>
            {step === 1 ? (
              <>
                <h2 className="text-black font-heading font-bold text-xl">Enter Booking Details</h2>
                <p className="text-silver text-xs">Choose pickup &amp; drop-off details</p>
              </>
            ) : (
              <>
                <h2 className="text-black font-heading font-bold text-xl">Book {car?.name || ''}</h2>
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={handleClose} className="text-gray-400 hover:text-black transition" aria-label="Close">
        ...
        {step === 1 && (
          <form onSubmit={handleContinue} className="p-5 space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <select
                  ...
                  className="w-full border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:border-gold focus:outline-none"
                >
                  {locations.map(loc => (
              ...
              <div>
                <label className="block text-black text-sm font-medium mb-1">Drop Off Location</label>
                <select
              ...
              <div>
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    min={today}
                    required
                    className="w-2/3 border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:border-gold focus:outline-none"
                  />
                  <input
                    type="time"
                    value={pickupTime}
                    onChange={(e) => setPickupTime(e.target.value)}
                    className="w-1/3 border border-gray-300 rounded-lg px-3 py-2.5 text-sm focus:border-gold focus:outline-none"
                  />
                </div>
              </div>
              <div>
                <label className="block text-black text-sm font-medium mb-1">Drop Off Date</label>
                <div className="flex gap-1">
                  <input
                    type="date"
                    value={dropoffDate}
            ...
            <button
              type="submit"
              className="w-full bg-gold hover:bg-gold-light text-black font-semibold py-3 rounded-lg transition text-sm"
            >
              Continue
            </button>
          </form>
          ...
          <form onSubmit={handleSubmit} className="divide-y divide-gray-100">
            ...
            <div className="p-5">
              <div className="flex flex-col md:flex-row gap-5">
                <div className="w-full md:w-48 h-36 bg-gray-100 rounded-lg flex items-center justify-center shrink-0">
                  {car.image ? (
                ...
                <div className="flex-1">
                  <h3 className="text-black font-heading font-bold text-xl">{car.name}</h3>
            ...
            <div className="p-5 grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <h4 className="text-black font-heading font-semibold text-sm mb-2">Booking Summary</h4>
                <div className="bg-gray-50 rounded-lg p-3 text-xs space-y-1 text-gray-600">
                  <p><span className="font-medium text-black">Pickup:</span> {locations.find(l => l.id === pickupLocation)?.name} on {pickupDate} at {pickupTime || '--:--'}</p>
                  <p><span className="font-medium text-black">Dropoff:</span> {locations.find(l => l.id === dropoffLocation)?.name} on {dropoffDate} at {dropoffTime || '--:--'}</p>
                  <p><span className="font-medium text-black">Total Price:</span> <span className="text-gold font-bold">{totalPrice}€</span> ({days} day{days > 1 ? 's' : ''})</p>
                </div>
                <h4 className="text-black font-heading font-semibold text-sm mt-4 mb-1">Description</h4>
                <p className="text-silver text-xs leading-relaxed">{car.description}</p>
              </div>

              {/* Your Information */}
              <div>
                <h4 className="text-black font-heading font-semibold text-sm mb-2">Your Information</h4>
                <div className="space-y-3">
                  <div>
                    <label className="block text-black text-xs font-medium mb-1">Full Name</label>
                    <input
                      type="text"
                      value={name}
                      onChange={(e) => setName(e.target.value)}
                      required
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-gold focus:outline-none"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-black text-xs font-medium mb-1">Email</label>
                      <input
                        type="email"
                        value={email}
                    ...
                    <div>
                      <label className="block text-black text-xs font-medium mb-1">Phone Number</label>
                      <input
                        type="tel"
                        value={phone}
                  ...
                  <div>
                    <label className="block text-black text-xs font-medium mb-1">Flight Number</label>
                    <input
                      type="text"
                      value={flightNumber}
                  ...
                  <div>
                    <label className="block text-black text-xs font-medium mb-1">Special Requests</label>
                    <textarea
                      value={requests}
                      onChange={(e) => setRequests(e.target.value)}
                      rows={2}
                      className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:border-gold focus:outline-none resize-none"
                    />
                  </div>
                </div>
              </div>
            ...
            {/* Available Extras */}
            <div className="p-5">
              <h4 className="text-black font-heading font-semibold text-sm mb-3">Available Extras</h4>
              <div className="space-y-2">
                {car.extras.map((extra) => (
                  <label key={extra.name} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 cursor-pointer">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                      ...
                      />
                      <span className="text-sm text-black">{extra.name}</span>
                    </div>
            ...
            <div className="p-5 space-y-3">
              ...
              <button
                ...
                className="w-full bg-gold hover:bg-gold-light text-black font-semibold py-3 rounded-lg transition text-sm disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Confirm Booking
              </button>
            </div>
        ...
        {showConfirmation && (
          <div className="p-10 text-center">
            ...
            <h3 className="text-black font-heading font-bold text-xl mb-6">Booking Confirmed!</h3>
            <button
              onClick={handleClose}
              className="bg-gold hover:bg-gold-light text-black font-semibold px-8 py-2.5 rounded-lg transition text-sm"
            >
              Done
            </button>
          </div>
